  ...
  const dispatchOrcamentoWebhook = async (id, status) => {
    try {
      ...
      if (!data?.dispatched) {
        ...
        toast({ title: 'Webhook com falha', description: data.results.filter((r) => !r.ok).map((r) => `${r.webhook_name}: ${r.error || `HTTP ${r.status}`}${r.response ? ` — ${r.response}` : ''}`).join(' · '), variant: 'destructive' });
      }
    } catch (e) {

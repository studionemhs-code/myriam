...
export default function AgentMessage({ message, onApprove, approvalBusy }) {
  ...
  return (
    <div className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${mine ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
        ...
        {message.pending_action && <ArchitectApprovalButton summary={message.pending_action.summary} onApprove={onApprove} busy={approvalBusy} />}

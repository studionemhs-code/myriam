  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1">
        ...
        {allowFiles && <label className="ml-auto cursor-pointer rounded-full p-2 text-muted-foreground hover:bg-muted" aria-label="Anexar arquivo"><Paperclip className="h-4 w-4" /><input type="file" className="hidden" accept=".pdf,.doc,.docx,.txt,.csv,.html,image/*,audio/*,video/*" onChange={(event) => setFile(event.target.files?.[0] || null)} /></label>}
      </div>
      ...
      <div className="flex items-center gap-2">
        <textarea rows={2} value={input} onChange={(event) => setInput(event.target.value)} placeholder={mode === 'text' ? 'Digite sua mensagem...' : 'Escreva ou grave sua mensagem...'} aria-label="Mensagem" className="min-w-0 max-h-40 flex-1 resize-y rounded-2xl border border-input bg-background px-4 py-2.5 text-sm outline-none focus:border-primary" disabled={busy} />

...
export default function AgentVoicePreview({ voice, agent }) {
  ...
  const preview = async () => {
    ...
    try {
      ...
      const result = await base44.integrations.Core.GenerateSpeech({ text: SAMPLE, voice, ...(useSavedAgent ? { agent_id: agent.id } : {}) });

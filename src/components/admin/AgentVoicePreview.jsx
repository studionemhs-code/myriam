...
export default function AgentVoicePreview({ voice, language, apiKey }) {
  ...
  useEffect(() => { setUrl(''); setError(''); }, [voice, language, apiKey]);
  const preview = async () => {
    ...
    try {
      ...
      const result = await base44.integrations.Core.GenerateSpeech({ text: SAMPLE, voice, ...(langCode ? { language_code: langCode } : {}), ...(apiKey?.trim() ? { api_key: apiKey.trim() } : {}) });
  ...
  return <div className="space-y-2 rounded-lg border border-border p-3">
    ...
    <p className="text-xs text-muted-foreground">O teste usa a chave digitada no agente; se o campo estiver vazio, usa a chave padrão segura do sistema. Esta prévia corresponde às mensagens de áudio; a conversa ao vivo pode usar outra voz.</p>
  </div>;
}
...
async function planChanges(request: string, operation: string, baseBranch: string) {
  ...
  return openAIJson(`Prepare uma alteração revisável no código do MYRIAM. Preserve tudo que não foi pedido. Para editar ou criar, devolva o conteúdo COMPLETO. Só use delete=true quando solicitado. Em suggest, apenas recomende e retorne changes vazio.\n\nOperação: ${operation}\nSolicitação: ${request}\n\n${files.map(file => `--- ${file.path} ---\n${file.content}`).join('\n\n')}`, 'architect_plan', {
